...
# ─────────────────────────────────────────────────────────────
# COLLECT METRICS — runs once per script execution
# st.rerun() at the bottom triggers the next refresh cycle
# ─────────────────────────────────────────────────────────────
...
prediction = predictor.predict(metrics, game_genre, resolution, preset, ray_tracing, upscaling)

# Generate recommendations ONCE per session (or after Refresh click)
        ...
        with st.container(border=True):
            ...
            with right_col:
                if rec_id in st.session_state.feedback_given:
                    ...
                    with fb1:
                        if st.button("👍", key=f"up_{rec_id}", help="This helped me!"):
                            db.update_recommendation_feedback(rec_id, True)
                            st.session_state.feedback_given[rec_id] = True
                            st.rerun()
                    with fb2:
                        if st.button("👎", key=f"dn_{rec_id}", help="Did not help"):
...
# This is the correct Streamlit pattern. No while True loop.
# Every widget is registered exactly once per script run.
# ─────────────────────────────────────────────────────────────
