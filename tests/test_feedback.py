...
def _get_first_rec_id() -> int | None:
    resp = client.post("/api/recommend", json=RECOMMEND_PAYLOAD)
    recs = resp.json().get("recommendations", [])
    return recs[0]["id"] if recs else None


# ── /api/feedback/summary ─────────────────────────────────────────────────────
def test_feedback_summary_returns_200():
    response = client.get("/api/feedback/summary")
    assert response.status_code == 200


def test_feedback_summary_has_status_success():
    response = client.get("/api/feedback/summary")
    assert response.json()["status"] == "success"


def test_feedback_summary_required_fields():
...
def test_feedback_summary_counts_non_negative():
    data = client.get("/api/feedback/summary").json()
...
# ── /api/feedback/{id} ────────────────────────────────────────────────────────
def test_feedback_submit_helpful():
    rec_id = _get_first_rec_id()
    if rec_id is None:
        pytest.skip("No recommendations returned — cannot test feedback")
    response = client.post(f"/api/feedback/{rec_id}", json={"was_helpful": True})
    assert response.status_code == 200
    assert response.json()["status"] == "success"


def test_feedback_submit_not_helpful():
    rec_id = _get_first_rec_id()
    if rec_id is None:
        pytest.skip("No recommendations returned — cannot test feedback")
    response = client.post(f"/api/feedback/{rec_id}", json={"was_helpful": False})
    assert response.status_code == 200
    assert response.json()["was_helpful"] is False


def test_feedback_response_contains_id():
    rec_id = _get_first_rec_id()
    if rec_id is None:
        pytest.skip("No recommendations returned — cannot test feedback")
    data = client.post(f"/api/feedback/{rec_id}", json={"was_helpful": True}).json()
    assert "id" in data
    assert data["id"] == rec_id


def test_feedback_updates_summary():
    """After submitting helpful feedback, helpful count should be >= 1."""
    rec_id = _get_first_rec_id()
    if rec_id is None:
        pytest.skip("No recommendations returned — cannot test feedback")
    client.post(f"/api/feedback/{rec_id}", json={"was_helpful": True})
    summary = client.get("/api/feedback/summary").json()
    assert summary["helpful"] >= 1


def test_recommend_response_includes_ids():
    """Every recommendation in POST /api/recommend must have an 'id' field."""
    data = client.post("/api/recommend", json=RECOMMEND_PAYLOAD).json()
